...
export default class Summary extends React.Component{
  ...
  _setMaxHeight(event){
    ...
    if (event.nativeEvent.layout.height > this.state.maxHeight || ! this.state.maxHeight){
      this.setState({
  ...
  _renderExpanded(){
    ...
    return (
      <View style={styles.container}>
        ...
        <View>
          <Text style={[styles.h4, {paddingHorizontal: 20,}]}>{longText}</Text>
        </View>
      </View>
    )
  }
    ...
    return (
      <Animated.View style={{backgroundColor: 'white', overflow: 'hidden', margin: 10, marginTop: 0, height: this.state.animation}}>
        ...
        <View onLayout={this._setMaxHeight.bind(this)} style={{position: this.state.measured ? 'absolute' : 'relative', opacity: this.state.measured ? 1 : 0}}>
          ...
          <Text style={[styles.h4, {}]}>{longText}</Text>
        </View>
      </Animated.View>
    )
  }
